    ...
    for endpoint in available_endpoints:
        try:
            if identity_version < 3:
                ...
                base_url = endpoint.get('url').replace('http://10.12.1.99','https://cloud.tcpisek.cz')
            if endpoint_type == "publicURL" else endpoint.get('url')
            base_url = endpoint.get('url').replace('http://10.12.1.100','https://cloud.tcpisek.cz') 
            if endpoint_type == "publicURL" else endpoint.get('url')
                return base_url
